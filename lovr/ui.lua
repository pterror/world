...
-- Partially embedded functions from https://github.com/meepen/Lua-5.1-UTF-8
...
--[[@param str string]]
--[[@param startPos? integer default=`1`]]
...
--[[@param str string]]
--[[@param startPos? integer default=`1`]]
--[[@param endPos? integer default=`-1`]]
...
--[[@param str string]]
--[[@param n integer]]
--[[@param startPos? integer default=`1` if `n >= 0` else default=`#str`]]
...
--[[@param lower_case string[] list of keys]]
--[[@param upper_case string[] list of keys]]
--[[@param symbols string[] list of keys]]
...
function UI.End(main_pass)
	...
	for i, v in ipairs(cur_window.command_list) do
		if v.type == "rect_fill" then
			if v.is_separator then
				...
				local m = lovr.math.newMat4(vec3(v.bbox.x + (cur_window.w / 2), v.bbox.y, 0),
					vec3(cur_window.w - (2 * _.margin), _.separator_thickness, 0))
				cur_window.pass:plane(m, "fill")
			else
				cur_window.pass:setColor(v.color)
				local m = lovr.math.newMat4(vec3(v.bbox.x + (v.bbox.w / 2), v.bbox.y + (v.bbox.h / 2), 0),
					vec3(v.bbox.w, v.bbox.h, 0))
				cur_window.pass:plane(m, "fill")
			end
	...
	local window_m = lovr.math.newMat4(cur_window.unscaled_transform:scale(cur_window.w * _.ui_scale,
		cur_window.h * _.ui_scale))
...
function UI.ProgressBar(progress, width)
	...
	if _.layout.same_line then
		bbox = {
			x = _.layout.prev_x + _.layout.prev_w + _.margin,
			y = _.layout.prev_y,
			w = width,
			h = (2 * _.margin) +
					text_h
	...
	table.insert(_.windows[#_.windows].command_list,
		{ type = "rect_wire", bbox = bbox, color = _.colors.progress_bar_border })
...
function UI.Separator()
	...
	table.insert(_.windows[#_.windows].command_list,
		{ is_separator = true, type = "rect_fill", bbox = bbox, color = _.colors.separator })
end
...
function UI.ImageButton(img_filename, width, height, text)
	...
	if not _.modal_window or (_.modal_window and _.modal_window == cur_window.id) then
		if _.PointInRect(_.last_off_x, _.last_off_y, bbox.x, bbox.y, bbox.w, bbox.h) and cur_window.id == _.hovered_window_id then
			_.hotID = my_id
			table.insert(_.windows[#_.windows].command_list,
				{ type = "rect_wire", bbox = bbox, color = _.colors.image_button_border_highlight })
			if _.input.trigger == _.e_trigger.pressed then
				_.activeID = my_id
			end
	...
	if text then
		table.insert(_.windows[#_.windows].command_list,
			{
				type = "image",
				bbox = { x = bbox.x, y = bbox.y + ((bbox.h - ib.h) / 2), w = ib.w, h = ib.h },
				texture = ib
						.texture,
				color = { 1, 1, 1 }
			})
		table.insert(_.windows[#_.windows].command_list,
			{
				type = "text",
				text = text,
				bbox = { x = bbox.x + ib.w, y = bbox.y, w = text_w + (2 * _.margin), h = bbox.h },
				color =
						_.colors.text
			})
	else
		table.insert(_.windows[#_.windows].command_list,
			{ type = "image", bbox = bbox, texture = ib.texture, color = { 1, 1, 1 } })
	end
...
function UI.WhiteBoard(name, width, height)
	...
	table.insert(_.windows[#_.windows].command_list,
		{ type = "image", bbox = bbox, texture = wb.texture, color = { 1, 1, 1 } })
...
function UI.TabBar(name, tabs, idx)
	...
	for i, v in ipairs(tabs) do
		...
		table.insert(_.windows[#_.windows].command_list,
			{ type = "rect_wire", bbox = tab_rect, color = _.colors.tab_bar_border })
		...
		if idx == i then
			table.insert(_.windows[#_.windows].command_list,
				{
					type = "rect_fill",
					bbox = { x = tab_rect.x + 2, y = tab_rect.y + tab_rect.h - 6, w = tab_rect.w - 4, h = 5 },
					color =
							_.colors.tab_bar_highlight
				})
		end
		x_off = x_off + tab_w
	end
...
function UI.Button(text, width, height)
	...
	elseif _.layout.same_column then
		bbox = {
			x = _.layout.prev_x,
			y = _.layout.prev_y + _.layout.prev_h + _.margin,
			w = (2 * _.margin) + text_w,
			h = (2 * _.margin) +
					(num_lines * text_h)
		}
		...
		bbox = {
			x = _.margin,
			y = _.layout.prev_y + _.layout.row_h + _.margin,
			w = (2 * _.margin) + text_w,
			h = (2 * _.margin) +
					(num_lines * text_h)
		}
...
function UI.TextBox(name, num_visible_chars, buffer)
	...
	if _.layout.same_line then
		bbox = {
			...
			y = _.layout.prev_y,
			w = (4 * _.margin) +
					(num_visible_chars * char_w) + label_w,
			h = (2 * _.margin) + text_h
		}
	else
		bbox = {
	...
	if _.focused_textbox and _.focused_textbox.id == my_id then
			...
			elseif osk.last_key == "right" then
				...
				if _.focused_textbox.cursor > _.focused_textbox.num_visible_chars + _.focused_textbox.scroll - 1 then
					...
					if _.focused_textbox.scroll > _.utf8.len(_.focused_textbox.text, 1, -1) - _.focused_textbox.num_visible_chars then
						_.focused_textbox.scroll = _.utf8.len(_.focused_textbox.text, 1, -1) - _.focused_textbox.num_visible_chars +
								1
					end
				end
				if _.focused_textbox.cursor > _.utf8.len(_.focused_textbox.text, 1, -1) then
					_.focused_textbox.cursor = _.utf8
							.len(_.focused_textbox.text, 1, -1)
				end
		...
		if #_.focused_textbox.text > 0 then
			...
			if str_len ~= #_.focused_textbox.text then
				if str_len >= num_visible_chars then
					str = _.utf8.sub(_.focused_textbox.text, _.focused_textbox.scroll,
						_.focused_textbox.scroll + num_visible_chars - 1)
				else
					--[[@diagnostic disable-next-line: param-type-mismatch]]
	...
	table.insert(_.windows[#_.windows].command_list,
		{ type = "text", text = name, bbox = label_rect, color = _.colors.text })
		...
		table.insert(_.windows[#_.windows].command_list,
			{
				type = "rect_fill",
				bbox = {
					x = text_rect.x + ((_.textbox_state[tb_idx].cursor - _.textbox_state[tb_idx].scroll + 1) * char_w) + _.margin +
							8,
...
function UI.ListBox(name, num_visible_rows, num_visible_chars, collection, selected)
	...
	if _.layout.same_line then
		bbox = {
			...
			y = _.layout.prev_y,
			w = (2 * _.margin) +
					(num_visible_chars * char_w),
			h = (num_visible_rows * text_h)
		}
	else
		bbox = {
			x = _.margin,
	...
	if lst_selected_idx >= lst_scroll and lst_selected_idx <= lst_scroll + num_visible_rows then
		...
		table.insert(_.windows[#_.windows].command_list,
			{ type = "rect_fill", bbox = selected_rect, color = _.colors.list_selected })
	end
	...
	if highlight_idx ~= nil then
		...
		table.insert(_.windows[#_.windows].command_list,
			{ type = "rect_fill", bbox = highlight_rect, color = _.colors.list_highlight })
	end
		...
		table.insert(_.windows[#_.windows].command_list,
			{
				type = "text",
				text = str,
				bbox = { x = bbox.x, y = y_offset, w = item_w + _.margin, h = text_h },
				color = _
						.colors.text
			})
...
function UI.SliderInt(text, v, v_min, v_max, width)
	...
	if _.focused_slider == my_id then
		...
		UI.SameLine()
		--[[@diagnostic disable-next-line: param-type-mismatch]]
		local __, bc, __, txt = UI.TextBox(text, (slider_w - (3 * _.margin)) / char_w, tostring(v))
	...
	table.insert(_.windows[#_.windows].command_list,
		{ type = "rect_fill", bbox = thumb_rect, color = _.colors.slider_thumb })
	table.insert(_.windows[#_.windows].command_list,
		{ type = "text", text = text, bbox = text_label_rect, color = _.colors.text })
	table.insert(_.windows[#_.windows].command_list,
		{ type = "text", text = v, bbox = text_value_rect, color = _.colors.text })
	return result, v
end
...
function UI.SliderFloat(text, v, v_min, v_max, width, num_decimals)
	...
	if _.focused_slider == my_id then
		...
		--[[@diagnostic disable-next-line: param-type-mismatch]]
		local __, bc, __, txt = UI.TextBox(text, (slider_w - (3 * _.margin)) / char_w, string.format(str_fmt, tostring(v)))
	...
	table.insert(_.windows[#_.windows].command_list,
		{ type = "text", text = text, bbox = text_label_rect, color = _.colors.text })
	table.insert(_.windows[#_.windows].command_list,
		{ type = "text", text = string.format(str_fmt, v), bbox = text_value_rect, color = _.colors.text })
	return result, v
end
...
function UI.CheckBox(text, checked)
	...
	table.insert(_.windows[#_.windows].command_list,
		{ type = "text", text = text, bbox = text_rect, color = _.colors.text })

	if checked and type(checked) == "boolean" then
		table.insert(_.windows[#_.windows].command_list,
			{ type = "text", text = "✔", bbox = check_rect, color = _.colors.check_mark })
	end
...
function UI.RadioButton(text, checked)
	...
	if checked and type(checked) == "boolean" then
		table.insert(_.windows[#_.windows].command_list,
			{ type = "circle_fill", bbox = check_rect, color = _.colors.radio_mark })
	end
